...
```text
theme.css 的基础色、用色角色及其明暗默认值（含别名与计算）
    → 元素、图标、集合条目、正文组件、首页
...
按变量引用检查，跨 CSS 文件的提供者主要就是 `theme.css` 和 `page-shell.css`；首页另消费模板生成的变量。全局、图标、列表、正文与首页的用色角色及其默认值集中到 `theme.css`，保留 `.icon`、`.collection-item-link`、`.prose`、`.page-home` 等作用域和必要的深色覆盖。组件不再定义配色公式，也不读取其他组件的专属颜色；正文的三类边框只共享一个中性基底，输出角色仍独立。此约定取代本文件历史审查中“组件颜色留在组件”的建议，站点覆盖契约见 [Assets](../assets/README.md#theme-colors)。
...
- 配色归属约定已更新：主题统一提供用色角色和默认值，保留 `.prose` 等作用域，不把所有变量提升到 `:root`；组件保留布局、动画和状态选择器。
