...
| 文件或文件组 | 负责什么 |
| --- | --- |
| `theme.css` | 基础色、语义用色角色及完整明暗默认值（含别名和计算）、浏览器 `color-scheme`；保留各变量的选择器作用域 |
...
用色角色及其默认值已集中到 `theme.css`：固定值、别名、混色公式与明暗差异在同一处定义，组件只消费颜色变量并拥有布局、透明度、动画和交互选择器。这取代前文历史方案中“组件专属配色留在组件”的建议，目标是让站点只覆盖一份主题资产即可调整所有主题用色。

原有组件用色变量归入主题，并补充标题、标题分隔线、删除线、引用细边、列表文字状态及彩色图标的角色。行内代码、表格、图片边框共享 `--prose-border-base`，保留各自输出变量和混色比例；单一用途的基底直接写在角色定义内，不再额外增加一层变量。`currentColor`、`inherit`、`transparent` 和浏览器控制的颜色不机械增加变量。现有明暗效果保持不变，代码高亮与首页信号仍可独立配色；当前公式不承诺任意强调色输入都有足够对比度。作用域、角色清单与站点覆盖方式见 [Assets](../assets/README.md#theme-colors)。

本次语义归拢验证：根站生产构建和 132 个 HTML 的产物检查通过，仍发布 6 个 CSS。内部 review 以分支起点 `cd4e94a` 的生产产物 `temp_workspace/public/2609121244-palette-before` 对比最终产物 `temp_workspace/public/2609121315-palette-semantics-after`：首页、WSL 文章、全部文章、全部产品、外观页在 390／1280px、明暗主题、默认／选中／hover／focus 下的 80 组计算颜色一致，20 组完整页面截图逐字节一致。Chromium 和 WebKit 各 12 项独立覆盖检查通过，验证角色可由计算改为色码、共享基底传播、组件状态互不误改及 SVG use 实例的实际重绘。

上述样式检查关闭业务脚本、冻结动画；另以正常脚本环境运行既有 `appearance-preference`、`prose-scope-and-hover-contract`，两项均通过，覆盖明暗／系统偏好切换、刷新、返回和跨标签同步，以及正文布局与 hover。报告分别位于 `temp_workspace/palette-internal-review-20260912/{result,webkit-overrides-result}.json`、`temp_workspace/palette-semantics-20260912/overrides-result.json` 和 `temp_workspace/regression/260912133122-browser/report.json`。

相对分支起点，整个重构的生产 gzip 公共 `page.css` 为 2,867 → 3,573 B（+706 B），正文为 2,208 → 1,726 B（−482 B），首页为 1,638 → 1,494 B（−144 B）；正文页两包合计增加 224 B，首页两包合计增加 562 B。颜色定义共用带来的开销有限，保留现有装配边界。

## 黑白灰的具体范围

建议页面、文字、导航、列表、控件及首页装饰收敛到黑白灰。灰色继续承担次要文字、层次和分隔，选中与焦点还有背景、描边、下划线等形态区别。

颜色按用途组织，先减少需要独立调节的色值，保留有意义的语义角色名。链接与正文、已访问与次要文字可以默认同色，但允许独立调整；列表状态、正文背景等默认值在 `theme.css` 中引用或计算。进一步派生配色时，应建立有用途分工的明暗色阶，再校验实际文字／背景组合，不能用一套固定混色比例替代对比度验证。
