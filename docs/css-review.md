...
原有组件用色变量归入主题，并补充标题、标题分隔线、删除线、引用细边、列表文字状态及彩色图标的角色。行内代码、表格、图片边框共享中性输入（归拢阶段名为 `--prose-border-base`，下述派生阶段改名为 `--prose-border-neutral`），保留各自输出变量和混色比例；单一用途的基底直接写在角色定义内，不再额外增加一层变量。`currentColor`、`inherit`、`transparent` 和浏览器控制的颜色不机械增加变量。此归拢阶段的明暗效果保持不变，代码高亮与首页信号仍可独立配色；当前公式不承诺任意强调色输入都有足够对比度。作用域、角色清单与站点覆盖方式见 [Assets](../assets/README.md#theme-colors)。
...
## 中性底色与边框派生（2026-09-12）

本轮继续保留棕色强调外观，将 11 处中性色码输入改为基于现有 `--bg`、`--fg` 的计算。中性混合决定与页面背景的距离，外层混合继续使用原来的强调色比例；不新增灰阶配置表，也不改变 `srgb` 混色空间。

| 角色或输入 | 浅色中 `--fg` 占比 | 深色中 `--fg` 占比 |
| --- | ---: | ---: |
| 共享 `--prose-border-neutral` | 18% | 15% |
| `--prose-rule-border`（无强调色） | 52% | 52% |
| 表头背景的中性输入 | 6% | 0%（直接使用页面背景） |
| 表格条纹的中性输入 | 2% | 0%（直接使用页面背景） |
| 代码块背景的中性输入 | 3% | 4% |
| 代码块边框的中性输入 | 12% | 15% |
| 代码高亮行的中性输入 | 7% | 9% |

行内代码、表格、图片边框共享 `--prose-border-neutral`，但各自保留独立输出角色。代码块边框的浅色处理不同，因此保留自己的计算关系；不因深色数值相同而绑定到共享输入。只使用一次的中性混合直接写在角色里。根级次要文字和边框、删除文字、语法高亮、首页信号色仍保留当前独立配色。

验证使用根站产物 `temp_workspace/public/2609121909-review-main` 与 `temp_workspace/public/260912-neutral-derived`：生产构建和 132 个 HTML 检查通过，仍为 6 个 CSS。公共 `page.css` 压缩源码为 15,919 → 16,344 B，gzip（level 9）为 3,699 → 3,704 B，仅增加 5 B。24 项明暗颜色比较中，经过浏览器绘制转为 8 位 sRGB 后，每个通道最大变化为 1/255。浅色、深色正文预览已检查，维持原有视觉层次。

新增 `palette-neutral-customization` 回归在旧产物上因边框不跟随前景输入而失败，在新产物的 Chromium 和 WebKit 中均通过；同时验证独立色码覆盖、作用域及恢复默认值。既有主题偏好与正文作用范围两项回归通过。对比记录在根站 `temp_workspace/derive-neutral-20260912/audit/`，浏览器报告分别为 `temp_workspace/regression/260912194021-browser/`、`260912194052-browser-webkit/` 和 `260912194049-browser/`。

46 组文字／背景抽样没有新增低于目标的组合，但原配色已有 4 项不足，尚未在本轮调整：

| 场景 | 修改前 | 修改后 | 普通文字目标 |
| --- | ---: | ---: | ---: |
| 浅色代码类型 | 4.492 | 4.495 | 4.5 |
| 浅色代码类型＋高亮行 | 4.051 | 4.051 | 4.5 |
| 浅色删除文字 | 2.849 | 2.849 | 4.5 |
| 深色关键词＋高亮行 | 4.265 | 4.261 | 4.5 |

所选根站文章缺少部分 token、删除文字和高亮行状态，这些情况使用临时 DOM 探针，并在报告中标明；未使用 exampleSite。对比度基于浏览器计算颜色和背景合成，8 位取样接近阈值时没有安全余量；例如深色高亮字符串为 4.515。以上是代表性配色抽样，不是全站可访问性认证。下一轮若调整文字配色，应覆盖普通与高亮背景，而非只检查单一底色；普通文字 4.5:1 的依据见 [WCAG 2.2 对比度说明](https://www.w3.org/WAI/WCAG22/Understanding/contrast-minimum.html)。

## 文字对比度修正（2026-09-12）

上一步记录的 4 个不足已修正。删除文字默认改为 `var(--muted)`，浅深模式都跟随次要文字，仍保留独立颜色角色与删除线角色；浅色类型改为 `#49673D`，深色关键词改为 `#C99E98`。深色高亮字符串原来仅略高于阈值，一并改为 `#BE9CD0`，为可读性留出余量。语法分类、原有绿／棕／紫色相及代码背景保持。

| 场景 | 修正前 | 修正后 |
| --- | ---: | ---: |
| 浅色代码类型 | 4.495 | 5.813 |
| 浅色代码类型＋高亮行 | 4.051 | 5.239 |
| 浅色删除文字 | 2.849 | 6.385 |
| 深色关键词＋高亮行 | 4.261 | 5.225 |
| 深色字符串＋高亮行（增加余量） | 4.515 | 5.242 |

基于 `temp_workspace/public/260912-text-contrast` 的相同 46 组抽样全部通过，没有缺失或不支持的采样；正文底色、边框及代码高亮背景的 24 项比较完全一致。浅深正文探针截图已检查；删除文字在深色模式下也会更明亮，与其它次要文字保持一致。报告和截图位于根站 `temp_workspace/contrast-fixes-20260912/`。

新增 `palette-text-contrast` 直接测量发布样式，覆盖 6 类代码文字的普通／高亮背景，以及普通段落、引用、表头、条纹行中的删除文字。34 个浅深组合要求普通文字至少 4.5:1，本次修正的角色至少 5:1；通过 Canvas 解析颜色并合成透明祖先背景，未把具体色码作为测试预期。旧产物因删除文字对比度不足而按预期失败，新产物在 Chromium 与 WebKit 均通过，同时重跑 `palette-neutral-customization` 验证覆盖契约。报告为 `temp_workspace/regression/260912210239-browser/` 和 `260912210239-browser-webkit/`。根站生产构建、132 个 HTML 产物检查通过。
