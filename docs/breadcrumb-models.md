...
文档只负责整页横向滚动；入口、路径列和右侧辅助列固定为 `15rem`，正文取视口可用宽度与 `88ch` 的较小值。第一列、每个路径列、正文和辅助列分别纵向滚动；`.page-content` 只承载正文，元信息位于其兄弟节点 `.document-aside`。辅助列随画布横向移动，正文滚动时保持原位；无需 fixed、sticky 或滚动同步脚本。集合表格按内容宽度扩展整页画幅，不增加内部横向滚动；正文中的表格与代码保留局部横向滚动。DOM 顺序与视觉顺序一致。

`.page-stage` 按 DOM 顺序排列路径、正文和辅助列，不再根据路径存在与否指定不同的列号。`slots.meta` 启用且元信息实际非空时才输出辅助列；章节列表尚未加入。元信息与正文标题共享 `--document-start-offset` 顶部偏移，长分类路径在列内换行。打印时各列展开，辅助信息排在正文后。`canvas-document-aside` 验证三语言、窄屏与宽屏、独立滚动、标题对齐、无空列和打印行为。
...
`browse/auto-scrollbar.js` 仅控制显隐：桌面正文列滚动时显示原生滚动条，停止 500ms 后使其透明；鼠标悬停在原生滚动条占用的范围内时保持显示，悬停正文不触发。不改变滚动条宽度或正文布局；触屏与强制颜色模式保持系统默认。接受 iOS 等系统的覆盖式滚动条，不额外增加正文右侧留白或偏移容器。禁用 JS 时仍可正常原生滚动。
