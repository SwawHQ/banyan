...
产品入口专项场景 `tools-category-entry-lineage` 验证四个分类的目录顺序，以及从全部／分类列表进入真实产品后的根选中、分类兄弟项、排序、刷新与前进后退。它从当前站点发现一个真实产品，不依赖固定产品 slug；至少需要一篇声明 `tools` 分类的工具正文。
...
站点已经移除全站语言推荐和 PWA 更新确认，不再注入“抑制推荐”的 localStorage 状态。语言场景以不同浏览器语言直接访问页面，验证首次访问和刷新都不会推荐或改写语言；更新场景验证静默发现 waiting worker、站内同标签页跳转前激活、其他标签页及输入保留、刷新/历史不额外重载、激活失败继续原跳转，以及检查页手动应用更新。
...
- `/all/`
- `/p/xvenv/?from=tools/first-party&sorts=_,name-asc`
- `/prefetchdebug`
