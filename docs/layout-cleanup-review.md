...
页面 shell 使用 `.page-shell`；路径、正文、右侧辅助列按 DOM 顺序自动排列，不再需要 `page-shell--has-path-columns` 布局分支。语言和“我的”的 stroke、fill、线宽等属性移入 `data/icons.toml`，与外观图标使用同一事实源。
...
README、taxonomy、列表、slots、发布迁移及导航计划已改为当前契约，不再把退役字段写成必填。`exampleSite` 当轮暂未修改，现已在后续专项清理中迁移到当前契约。
